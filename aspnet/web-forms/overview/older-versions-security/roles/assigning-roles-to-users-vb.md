...
> [!NOTE]
> `Linq.Enumerable.Contains(Of String)(...)`構文は、ASP.NET 2.0 を使用している場合はコンパイルされません。 `Contains(Of String)`メソッドの一部である、 [LINQ ライブラリ](http://en.wikipedia.org/wiki/Language_Integrated_Query)、ASP.NET 3.5 に新機能です。 ASP.NET version 2.0 を使用して引き続き場合を使用して、 [ `Array.IndexOf(Of String)`メソッド](https://msdn.microsoft.com/en-us/library/eha9t187.aspx)代わりにします。
