...
- **1 つまたは複数を新規作成 * * *`WizardStep`* * * 追加のユーザー インターフェイス要素を格納する s**です。 新しいを追加する`WizardStep`、CreateUserWizard をクリックして、"追加/削除`WizardSteps`"を起動するスマート タグからのリンク、`WizardStep`コレクション エディターです。 そこから追加、削除、またはウィザードの手順の順序を変更することができます。 これは、このチュートリアルのために使用するアプローチです。
