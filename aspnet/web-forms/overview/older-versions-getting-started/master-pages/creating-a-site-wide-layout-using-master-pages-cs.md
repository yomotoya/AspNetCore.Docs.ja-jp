...
<strong>図 01</strong>: www.asp.net の web サイトを使用して、一貫した参照と思われる間でのすべてのページ ([フルサイズのイメージを表示するをクリックして](creating-a-site-wide-layout-using-master-pages-cs/_static/image3.png))
...
宣言型マークアップの最初の行は、 [ `@Master`ディレクティブ](https://msdn.microsoft.com/library/ms228176.aspx)です。 `@Master`ディレクティブがに似ていますが、 [ `@Page`ディレクティブ](https://msdn.microsoft.com/library/ydy4x04a.aspx)ASP.NET ページに表示されます。 これは、サーバー側の言語 (c#) と、マスター ページの分離コード クラスの継承と場所に関する情報を定義します。
