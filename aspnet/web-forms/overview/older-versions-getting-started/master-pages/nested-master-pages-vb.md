...
唯一のコンテンツ ページを使用する、`~/Admin/AddProduct.aspx`からマスター ページを派生`~/Admin/Products.aspx`します。 前のチュートリアルで作成した、 `MustInherit` `BaseMasterPage`クラスが、これらのパブリック メンバーを定義します。
