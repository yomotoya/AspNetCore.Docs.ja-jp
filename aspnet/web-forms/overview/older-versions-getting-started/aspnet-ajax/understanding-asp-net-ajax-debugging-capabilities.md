...
> *> [!NOTE] Windows Vista ユーザー アクセス制御 (UAC) が有効になっているを実行して、Visual Studio 2008 が、管理者として実行する設定がある場合、アタッチするメッセージが表示されたら、プロセスにアタッチする Visual Studio は失敗します。この問題を回避するには、最初に、Visual Studio を起動し、そのインスタンスを使用してデバッグします。*
