...
既定の web プロジェクトがたくさん削除もでオーバーヘッドののみが含まれています、Default.aspx ページ、default.cs ファイル、およびアプリ\_データ フォルダーです。 Web.config および特別なフォルダー (つまりアプリ\_コード) に必要に応じて追加されます。 Web プロジェクトは、ファイルとする必要があるフォルダーに含まれています。
...
> [!NOTE]
> Vs ###\_tmp.htm ファイルと get\_aspx\_ver.aspx 不要にバージョン情報を決定します。
...
> [!NOTE]
> 一時フォルダーの場所は、c:\Documents and Settings\&lt;ユーザー&gt;\Local Settings\Temp\VWDWebCache\&lt;サーバー&gt;\_&lt;アプリケーション名&gt;
...
> [!NOTE]
> 型の読み込みできなかったことを示すエラーが発生したかどうかは\_既定では、Web サイトと以前のバージョンではなくで ASP.NET 2.0 を実行していることを確認してください。 インターネット インフォメーション サービスで、[ASP.NET] タブから、ことを行うことができます。
...
C:\MyWebSite\Nested 別の web プロジェクト

C:\MyWebSite で Web サイトを開いたときに、入れ子になったフォルダーは、そのアプリケーションのサブ フォルダーとして表示されます。
...
C:\Documents and Settings\&lt;ユーザー&gt;\Local Settings\Temp\VWDWebCache\&lt;サーバー&gt;\_&lt;アプリケーション名&gt;
...
2003 と Visual Studio の 2002 では、Web アプリケーションが/bin フォルダー内に存在する 1 つのプライマリ アセンブリにコンパイルされました。 Visual Studio 2005 で、アプリ\_コード フォルダーが追加されました。 クラスとその他の非 UI コードは、アプリに追加されます\_コード フォルダーです。 Visual Studio がアプリですべてのファイル、プロジェクトをビルドするときに\_コード フォルダーは、1 つのアプリにコンパイル\_Code.dll ファイル。 この変更の結果は、後続のビルドが以前のバージョンよりも高速です。
...
Visual Studio のコンパイル モデル内の別の向上は、Web サイトの発行の機能です。 パブリッシュ機能は、Web サイトをプリコンパイル、ために、開発者は要求時に何もコンパイルする必要がないの追加されたパフォーマンスを利用できます。 アプリ内のすべてのソース コードをプリコンパイルも\_コード DLL のフォルダーを展開するソース コードを持たないようにします。
...
> [!NOTE]
> Aspnet\_compile.exe ユーティリティを使用して事前に、ASP.NET Web アプリケーションをコンパイルすることもできます。 ツールは、モジュール 9 で取り上げます。
...
1. リモートの Web サーバーから UNC パスを使用して開発用コンピューターで msvsmon.exe を検索し、それを実行します。  
 Msvsmon.exe を既定の場所は\\server\c$ \Program Files\Microsoft Visual Studio 8\Common7\IDE\Remote Debugger\x86 です。
2. リモート デバッグ用のポートのブロックを解除するメッセージが表示されたら、ようになります。
3. 開発用コンピューターから Default.aspx の分離コードを開き、ページにブレークポイントを設定\_Load メソッドです。
...
C:\Windows\Microsoft.NET\Framework\v2.0 にある WebDev.WebServer.exe ファイルを実行して、コマンドラインを使用して、ASP.NET 開発サーバーを起動できます。\*\*\*\*\*. 次のダイアログ ボックスには、使用可能なパラメーターが表示されます。
