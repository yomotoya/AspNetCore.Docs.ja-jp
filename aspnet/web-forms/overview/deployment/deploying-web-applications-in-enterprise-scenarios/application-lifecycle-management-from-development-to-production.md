...
> 
> トピックの概要を説明するものでは、[一連のチュートリアル](deploying-web-applications-in-enterprise-scenarios.md)企業内の web 配置でします。 ここで説明する概念 & #x 2014 の一部をあまり詳しくない場合、心配しないでください以外の場合は次のチュートリアルは、これらのタスクと手法のすべての詳細情報を提供します。
> 
> > [!NOTE]
...
> いずれの場合、最終的な結果は、同じ & #x 2014 です。MSBuild では、マージされたプロジェクト ファイルを実行し、ターゲット環境にソリューションを展開します。 これにより、発行プロセスをトリガーする方法の柔軟性が大幅に向上します。
...
内で、 *Publish.proj*ファイル、 **WhatIf**プロパティは、"what-if"モードで展開のすべてのリソースを発行することを示します。 つまり、展開に出かけてしまってが展開先の環境で何も変更が実際には、ログ ファイルが生成されます。 提案された配置 & #x 2014 の影響を評価できます。 具体的には、新機能は、追加、取得とは、更新、および新機能が削除されてしまう & #x 2014; 実際に変更を加える前にします。
