...
チュートリアルおよびその他の情報は、ASP.NET および Web ツール 2012.2 は ASP.NET web サイトから使用可能な (https://www.asp.net)です。
...
- 選択的発行&#8211;の 1 つまたは複数のファイルは、(Web 配置のエンドポイントへの発行) した後、次の操作を行うことができます。 
...
Facebook Application テンプレートにより、簡単にユーザーの Facebook からのリアルタイムの更新を処理する Web API コント ローラーを作成します。 開発用コンピューターが NAT の内側にある場合は、コント ローラーがネットワーク構成を変更しない動作しない可能性があります。 詳細については、ここで参照してください。 [http://facebook.stackoverflow.com/questions/5259467/can-a-computer-behind-a-nat-router-receive-realtime-updates-from-facebook](http://facebook.stackoverflow.com/questions/5259467/can-a-computer-behind-a-nat-router-receive-realtime-updates-from-facebook)
