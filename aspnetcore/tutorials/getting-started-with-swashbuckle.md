...
アプリを起動し、`http://localhost:<random_port>/swagger/v1/swagger.json` に移動します。 [Swagger 仕様 (swagger.json)](xref:tutorials/web-api-help-pages-using-swagger#swagger-specification-swaggerjson) に基づき、エンドポイントについて説明するドキュメントが生成され、表示されます。

Swagger UI は `http://localhost:<random_port>/swagger` にあります。 Swagger UI から API を探し、他のプログラムに組み込みます。

> [!TIP]
> アプリのルート (`http://localhost:<random_port>/`) で Swagger UI にサービスを提供するには、`RoutePrefix` プロパティを空の文字列に設定します。
> 
