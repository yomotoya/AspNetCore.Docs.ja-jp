...
`VirtualPathData.VirtualPath` プロパティには、ルートによって生成された*仮想パス*が含まれます。 必要に応じて、パスをさらに処理する必要がある場合があります。 たとえば、生成された URL を HTML で表示するには、アプリケーションの基礎パスを先頭に追加する必要があります。
...
ルーティングで使用される正規表現は、多くの場合、`^` 文字で始まり (文字列の開始位置に一致)、`$` 文字で終わります (文字列の終了文字に一致)。 `^` 文字と `$` 文字により、正規表現はルート パラメーター値全体に一致します。 `^` 文字と `$` 文字がなければ、正規表現は、意図に反して文字列内のあらゆる部分文字列に一致します。 下の表では例をいくつか挙げ、それが一致する/一致しない理由をまとめています。
