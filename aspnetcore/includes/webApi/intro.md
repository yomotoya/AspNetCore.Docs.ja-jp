...
* クライアントとは、Web API を使用するもの (モバイル アプリやブラウザーなど) です。 このチュートリアルでは、クライアントは作成しません。 アプリのテストでは、[Postman](https://www.getpostman.com/) または [curl](https://curl.haxx.se/docs/manpage.html) をクライアントとして使用します。

* *モデル*は、アプリのデータを表すオブジェクトです。 ここでは、唯一のモデルが to-do 項目です。 モデルは C# クラスで表され、POCO (**P**lain **O**ld **C**LR **O**bject。つまり、単純な従来の CLR オブジェクト) とも呼ばれます。
