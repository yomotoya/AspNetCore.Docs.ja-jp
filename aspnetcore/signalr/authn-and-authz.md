...
[サンプル コードのダウンロードを表示または](https://github.com/aspnet/AspNetCore.Docs/tree/master/aspnetcore/signalr/authn-and-authz/sample/) [(ダウンロードする方法)](xref:index#how-to-download-a-sample)
...
次の例に示します`Startup.Configure`SignalR と ASP.NET Core の認証を使用します。
...
> [!NOTE]
> SignalR と ASP.NET Core 認証ミドルウェアを登録する順序は重要です。 常に呼び出す`UseAuthentication`する前に`UseSignalR`SignalR のユーザーがあるできるように、`HttpContext`します。
