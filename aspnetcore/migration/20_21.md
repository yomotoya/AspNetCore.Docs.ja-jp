...
A*共有フレームワーク*アセンブリのセットです (*.dll*ファイル)、アプリのフォルダーにはないです。 共有フレームワークは、アプリを実行するコンピューターにインストールする必要があります。 詳細については、次を参照してください。[共有フレームワーク](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/)します。
