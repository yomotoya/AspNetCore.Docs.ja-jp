...
> [!NOTE]
> コント ローラーは、多数の責任で過度に複雑べきではありません。 コント ローラーのロジックが過度に複雑になることを防止するには、[単一責任の原則](http://deviq.com/single-responsibility-principle/)プッシュ ビジネス ロジック、コント ローラー アウトし、ドメイン モデルにします。
