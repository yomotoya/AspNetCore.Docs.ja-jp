---
title: "Web API の構築"
author: rick-anderson
description: 
keywords: ASP.NET Core,
ms.author: riande
manager: wpickett
ms.date: 01/05/2016
ms.topic: article
ms.assetid: a8fb7eb7-e0e5-4394-84f3-1f1dbe0ba2ef
ms.technology: aspnet
ms.prod: asp.net-core
ms.openlocfilehash: 42dabfcef4aeae2505c1998d75ec700429cb2f21
ms.sourcegitcommit: fe25b8163e9e4115915b3293936e9da646fff84f
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 10/09/2017
---
# <a name="building-web-apis"></a>Web API の構築

* [Visual Studio を使用して ASP.NET Core MVC で初めての Web API を作成する](../../tutorials/first-web-api.md)
* [Swagger を使用する ASP.NET Core Web API のヘルプ ページ](../../tutorials/web-api-help-pages-using-swagger.md)
* [ネイティブ モバイル アプリケーションのバックエンド サービスの作成](../../mobile/native-mobile-backend.md)
* [応答データの書式設定](../models/formatting.md)
* [カスタム フォーマッタ](../advanced/custom-formatters.md)

