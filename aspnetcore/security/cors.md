...
ブラウザーのセキュリティは、web ページが別のドメインに AJAX 要求を行うことを防止します。 この制限は *同一生成元ポリシー* と呼ばれ、悪意のあるサイトが別のサイトから機密データを読み取ろうとすることを防いでいます。 しかし、時には、他のサイトからあなたの web API へ クロス オリジン要求を作成させたいこともあるでしょう。
