...
ASP.NET Core モジュール インストーラーの実行の権限を持つ、**システム**アカウント。 インストーラーが、アクセス拒否エラーのモジュールの設定を構成しようとするときに達するため、ローカル システム アカウントには、IIS 共有構成で使用される共有のパスに対するアクセス許可を変更しませんが、 *applicationHost.config*共有します。
