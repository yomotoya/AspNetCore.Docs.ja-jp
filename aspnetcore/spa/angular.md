...
> [!NOTE]
> このドキュメントがリリースされた角度のプロジェクト テンプレートについて **このドキュメントは、角度のテンプレートのリリース候補です。** 初期 2018 でリリースされたバージョンを出荷する予定です。
...
開発モードでこのコードがスクリプトを実行して、SSR バンドルが作成されます`build:ssr`で定義されている*ClientApp\package.json*です。 これにより、ビルドという名前の角度アプリ`ssr`は未定義です。 
